...
- [ ] Preserve `[x]` checkbox state when re-pulling a meeting that already has a `# Meeting Summary` section. Today the merger replaces the entire section on every pull, so any checkmarks the user has ticked off in Action Items revert to `[ ]`. Path: skill (or merger) reads existing checkbox state for each action item line and re-applies it to the regenerated content via fuzzy text match. Low priority — re-pulls of the same meeting are rare in normal flow; this only bites when the user has done curation work inside Meeting Summary AND chooses to regenerate. Risk: fuzzy matching on action item text is brittle (item wording can drift between pulls).
