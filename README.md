...
- **Merger** (`merger.py`): When the resolved target already exists, the push merges instead of overwriting. Top-level sections are H1 — tool-owned (`# Meeting Summary` at the top of the body, then `# Notes` / `# Enhanced Notes` / `# Transcript` at the bottom) are replaced; user-owned content (`# Prep Notes` and anything nested within it, custom H1 sections, free preamble) is preserved. Frontmatter merges on the same principle: tool-owned fields (`date`, `meeting-title`, `attendees`) update from the new render when non-empty; everything else stays put. Use `--force` for a clean overwrite at the default path.
- **Meeting Summary** (`renderer.py`): When the caller passes `--meeting-summary` / `--meeting-summary-file`, the renderer prepends a `# Meeting Summary` section above the first existing H1. The `/pull-granola-notes` skill generates this content from the transcript during the pull (Summary + Key Decisions + Action Items, with attribution caveats for in-room speakers). Omitted when the caller provides nothing — re-pulls without a fresh summary preserve any prior `# Meeting Summary` already in the file.
- **Migration** (`migrate.py`): One-time helper that bumps tool-section headings from H2 to H1 in files written before the H1 cutover. Drops legacy `# {title}` lines (the title is in frontmatter and the filename), renames legacy `# Notes` H1 (the old prep-content wrapper) to `# Prep Notes`, and only promotes EXACT tool-section headings — user H2 sub-headings inside prep content are left alone. Idempotent; skips Obsidian index files prefixed with `*`. Exposed as `python3 -m granola_sync migrate-headings --folder PATH [--apply]`.
